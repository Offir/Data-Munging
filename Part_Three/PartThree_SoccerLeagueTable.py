...
from PartThree import finds_minimum_difference
from PartThree import print_result
...
class SoccerTeam(object):
    def __init__(self, team_name, for_goals, against_goals):
        ...
        self.difference = abs(for_goals - against_goals)
...
def object_decoder(line_from_file):
    ...
    if check_validity_of_the_object(team_for_goals, team_against_goals, team_name) and\
        ...
        soccer_team =  SoccerTeam(team_name, int(team_for_goals), int(team_against_goals))
    elif team_name == TEAM:
        soccer_team = None
    else:
